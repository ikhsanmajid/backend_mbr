...
async function check_category(data) {
    try {
...
async function update_kategori(id, putData) {
    try {
...
async function delete_kategori(id) {
    try {
...
async function accept_permintaan(data) {
    try {
        const resultAcceptPermintaan = await prisma.$transaction(async (tx) => {
            ...
            for (const detail of findRB) {
                ...
                const lockKey = BigInt(year) * 1000000000n +
                    BigInt(kategori) * 1000000n +
                    BigInt(jenis);
...
//ANCHOR - Update Permintaan RB
async function reject_permintaan(data) {
    try {
...
async function get_permintaan(data) {
    try {
        const year = new Date().getFullYear();
        const query = `SELECT 
            r."id",
...
async function get_permintaan_bagian(data) {
    try {
        const searchRequest = await prisma.$queryRaw `SELECT
            p."id",
            p."namaProduk",
            COUNT(CASE WHEN n."tanggalKembali" IS NULL THEN 1 END) AS "RBBelumKembali",
            COUNT(CASE WHEN n."tanggalKembali" IS NOT NULL THEN 1 END) AS "RBSudahKembali",
...
async function get_nomor_by_id(data) {
    try {
        const searchRequest = await prisma.$queryRaw `
                SELECT
            n."id",
...
async function get_permintaan_by_id(data) {
    try {
...
async function get_nomor_permintaan_by_id(data) {
    try {
        const searchRequest = await prisma.$queryRaw `
                SELECT
            d."id",
...
async function get_rb_return_by_bagian(id, status, numberFind, limit, offset, startDate, endDate) {
    //console.log(status)
    try {
        let query = `SELECT
            d."idPermintaanMbr" AS id,
...
//ANCHOR - Get Permintaan RB Return Berdasarkan Produk
async function get_rb_return_by_status_outstanding(numberFind, limit, offset, startDate, endDate) {
    try {
        let query = `SELECT
            d."idPermintaanMbr" AS "id",
            d."idProduk",
...
async function confirm_rb_return(id, idAdmin) {
    try {
        const updateRequest = await prisma.nomormbr.update({
            where: {
                id: id
...
async function get_laporan_rb_belum_kembali_perbagian(idBagian, status, startDate, endDate) {
    try {
        const year = new Date().getFullYear();
        const query = `SELECT
            n."nomorUrut",
...
async function generate_report_dashboard_admin() {
    try {
...
//ANCHOR - Generate Report Dashboard Admin
async function generate_report_pembuatan_rb(tahun) {
    try {
        const query = `SELECT
            j."namaJenisBagian",
        ...
        const request = await prisma.$queryRaw(client_1.Prisma.sql([query]));
        if (request.length == 0) {
            return { data: [] };
        }
